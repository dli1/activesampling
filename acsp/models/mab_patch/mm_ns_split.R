...
mm_ns_multiple_queries_split <- function(pool_folder, out_folder, qrels_path, indice_path, index)
{
  ...
  # read train system runs
  all_runs = read.table(indice_path, header=FALSE, sep=",", na.strings="NaN", col.names=c("INDEX","TRAIN_TEST","RUN_NAME"))

  train_runs = (all_runs[which(all_runs$INDEX==index&all_runs$TRAIN_TEST=="train"), ])$RUN_NAME
